...
import { FANS, CHATTERS } from "@/lib/mock-data";
import { DollarSign, Zap, Image, MessageSquare, Clock } from "lucide-react";
import { formatDistanceToNow } from "date-fns";
import { MessageDetailDrawer } from "@/components/ui/message-detail-drawer";
...
export function ActivityFeed({ transactions, limit = 20 }: ActivityFeedProps) {
  ...
  const handleTransactionClick = (transaction: Transaction) => {
    // Only open drawer for message transactions with content
    if (transaction.type === "message" && transaction.messageContent) {
  ...
  const handleCloseDrawer = () => {
    ...
    // Delay clearing transaction until after animation
    setTimeout(() => setSelectedTransaction(null), 300);
  };

  // Get fan and chatter data for the selected transaction
  const selectedFan = selectedTransaction
    ? FANS.find((f) => f.id === selectedTransaction.fanId)
    : undefined;
  const selectedChatter = selectedTransaction?.chatterId
    ? CHATTERS.find((c) => c.id === selectedTransaction.chatterId)
    : undefined;
      ...
      <div className="space-y-1">
        <div className="max-h-[500px] space-y-1 overflow-y-auto pr-2 scrollbar-thin scrollbar-track-slate-800/30 scrollbar-thumb-slate-700/50">
          {displayTransactions.map((transaction) => {
            ...
            const hasMessageContent = transaction.type === "message" && transaction.messageContent;
            ...
            return (
              <div
                ...
                className={`group flex items-center gap-3 rounded-lg border border-slate-800/30 bg-slate-900/20 p-3 transition-all hover:border-slate-700/50 hover:bg-slate-800/30 ${
                  hasMessageContent ? "cursor-pointer hover:scale-[1.01]" : ""
                }`}
              >
                {/* Icon */}
